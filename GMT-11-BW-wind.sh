...
gmt grdcut TerraClimate_ws_2018.nc -R19.5/29.5/-27/-17.5 -Gbw_wind.nc
gdalinfo -stats bw_wind.nc
# Minimum=1.800, Maximum=3.400, Mean=2.587, StdDev=0.352

# Make color palette
# gmt makecpt -Cspellbound -V -T1.8/3.4 > pauline.cpt
gmt makecpt -Csaga-17.cpt -V -T1.8/3.4 > pauline.cpt

ps=BW_wind.ps
gmt grdimage bw_wind.nc -Cpauline.cpt -R19.5/29.5/-27/-17.5 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour bw_wind.nc -R -J -C0.5 -A0.3 -Wthin,white -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"WS (Wind speed) in Botswana (2018)" -O -K >> $ps
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,0,black+jLB -Gwhite@40 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f17p,19,black+jLB -Gwhite@60 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f13p,23,blue2+jLB -Gwhite@40 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f13p,23,blue2+jLB+a-45 -Gwhite@40 >> $ps << EOF
22.1 -18.45 Okavango
EOF
gmt pstext -R -J -N -O -K \
-F+f14p,23,darkgreen+jLB -Gwhite@40 >> $ps << EOF
23.9 -18.6 Chobe
23.5 -18.9 National Park
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,23,blue2+jLB+a-320 -Gwhite@40 >> $ps << EOF
26.1 -24.37 Ngotwane
EOF
gmt pstext -R -J -N -O -K \
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,23,blue2+jLB+a-343 -Gwhite@40 >> $ps << EOF
28.2 -22.57 (Limpopo)
EOF
gmt pstext -R -J -N -O -K \
-F+f13p,23,blue2+jLB+a-350 -Gwhite@40 >> $ps << EOF
22.5 -22.38 Okwa
EOF
...
# Add subtitle
gmt pstext -R0/10/0/15 -JX10/10 -X0.0c -Y11.6c -N -O \
...
gmt psconvert BW_wind.ps -A0.5c -E720 -Tj -Z
